...
const Execute_ETH_Transfer = ({ id }) => {
  ...
  const handleDepositChange = async (e) => {
    ...
    if(axelarFee == 0) axelarFee = 20500000;
    setAxelarFee(parseFloat(ethers.utils.formatUnits(axelarFee.toString(), "mwei")).toFixed(2));
    const expected = e.target.value - ethers.utils.formatUnits(axelarFee.toString(), "mwei");
  ...
  const handleMax = async (e) => {
    ...
    const expected = accbalance - ethers.utils.formatUnits(axelarFee.toString(), "mwei");
    setExpected(parseFloat(expected).toFixed(2));    
  }  
  ...
  const handleDepositSubmit = async (e) => {
    ...
    else
    {
      ...
      try {
        const result = await tokenContract.transfer(depositAddressValue, dx);
        await result.wait();
        setSuccess(true);
      ...
      <div className="container">   
        ...
        <hr></hr>
          ...
          { keplraddress ? <span className="box bRight"><img src="../images/Keplr.png"></img>Keplr Connected</span> : <button type="button" onClick={connectKeplr} className="btn btn-primary bRight"><img src="../images/Keplr.png"></img>Connect Keplr</button> }            
            ...
            <p className='fees'><span className='secTwo'><b>Axelar Fees:</b> {axelarFee} USDC</span></p>
            <hr></hr>
            ...
            <form onSubmit={handleDepositSubmit} >
              ...
              <button type="submit" disabled={execute ? true : false } className="btn btn-primary"><span className={executeVal}></span> Execute</button>
...
export default Execute_ETH_Transfer;